...
for n, name in enumerate(expts_name):
    if os.path.isfile(os.path.join(args.mcmc_dir, name, args.map_file)):
        ...
        # for key in ['log_sigma_CRC:ESI:0', 'log_sigma_CRC:ESI:1', 'log_sigma_CRC:ESI:2', 'log_sigma_CRC:ESI:3']:
        for key in [f'log_sigma_CRC:{name}:0', f'log_sigma_CRC:{name}:1', f'log_sigma_CRC:{name}:2', f'log_sigma_CRC:{name}:3']:
            if key in map.keys():
                combined_map['log_sigma_CRC:'+name+':'+key[-1]] = map[key]

combined_map['log_sigma_CRC:ES:0'] = map['log_sigma_CRC:ES:0']
combined_map['log_sigma_CRC:ES'] = map['log_sigma_CRC:ES:0']
