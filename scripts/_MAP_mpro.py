...
from _MAP import _extract_logK_kcat, _uniform_pdf, _gaussian_pdf, _lognormal_pdf, _log_normal_likelihood, _map_adjust_trace, _log_prior_sigma
...
def _log_priors(mcmc_trace, experiments, prior_infor, nsamples=None,
    ...
    for key in mcmc_trace.keys():
        ...
        if key.startswith('alpha'):
            log_priors += jnp.log(_uniform_pdf(mcmc_trace[key][: nsamples], 0.5, 1.5))
...
def _log_likelihood_each_dataset(type_expt, data, trace_logK, trace_kcat, trace_alpha,
                                 trace_sigma, trace_error_E, in_axes_nth, nsamples):
    """
    Parameters:
    ----------
    ...
    if type_expt == 'kinetics':
        ...
        f = vmap(lambda logKd, logK_S_M, logK_S_D, logK_S_DS, logK_I_M, logK_I_D, logK_I_DI, logK_S_DI, kcat_MS, kcat_DS, kcat_DSI, kcat_DSS, sigma: _log_normal_likelihood(rate,
                                                                                                                                                                            ReactionRate(kinetics_logMtot, kinetics_logStot, kinetics_logItot, logKd, logK_S_M, logK_S_D, logK_S_DS,
                                                                                                                                                                                         logK_I_M, logK_I_D, logK_I_DI, logK_S_DI, kcat_MS, kcat_DS, kcat_DSI, kcat_DSS),
                                                                                                                                                                            sigma),
    ...
    if type_expt == 'AUC':
        ...
        f = vmap(lambda logKd, logK_S_M, logK_S_D, logK_S_DS, logK_I_M, logK_I_D, logK_I_DI, logK_S_DI, sigma: _log_normal_likelihood(auc,
    ...
    if type_expt == 'ICE':
        ...
        f = vmap(lambda logKd, logK_S_M, logK_S_D, logK_S_DS, logK_I_M, logK_I_D, logK_I_DI, logK_S_DI, kcat_MS, kcat_DS, kcat_DSI, kcat_DSS, sigma: _log_normal_likelihood(ice,
    ...
    if type_expt == 'CRC':
        [rate, kinetics_logMtot, kinetics_logStot, kinetics_logItot] = data
        f = vmap(lambda logKd, logK_S_M, logK_S_D, logK_S_DS, logK_I_M, logK_I_D, logK_I_DI, logK_S_DI, kcat_MS, kcat_DS, kcat_DSI, kcat_DSS, alpha, sigma, error_E: _log_normal_likelihood(rate,
...
def _log_likelihood_each_expt(type_expt, expt, idx_expt, mcmc_trace, idx, nsamples):
    """
    Parameters:
    ----------
    type_expt       : str, 'kinetics', 'AUC', or 'ICE'
    ...
    if type(expt[type_expt]) is dict:
        for n in range(len(expt[type_expt])):
            ...
            if data is not None:
                ...
                log_likelihoods += _log_likelihood_each_dataset(type_expt=type_expt, data=data,
                                                                trace_logK=trace_nth, trace_kcat=trace_nth, trace_alpha=None, 
                                                                trace_sigma=trace_sigma, trace_error_E=None,
...
def _log_likelihoods(mcmc_trace, experiments, alpha_list=None, E_list=None, nsamples=None, show_progress=True):
    ...
    for idx, expt in enumerate(experiments):
            ...
            if type(expt['CRC']) is dict:
                for n in range(len(expt['CRC'])):
                    ...
                    if data_rate is not None and 'plate' in expt.keys():
                        ...
                        log_likelihoods += _log_likelihood_each_dataset('CRC', data_rate, trace_nth, trace_nth, trace_alpha, trace_sigma, trace_error_E, in_axis_nth, nsamples)
                ...
                if data_rate is not None and 'plate' in expt.keys():
                    ...
                    log_likelihoods += _log_likelihood_each_dataset(type_expt='CRC', data=data_rate,
        ...
        for _type_expt in ['kinetics', 'AUC', 'ICE']:
            if _type_expt in expt.keys():
                log_likelihoods += _log_likelihood_each_expt(_type_expt, expt, idx_expt, mcmc_trace, idx, nsamples)

    return np.array(log_likelihoods)


def _map_finding(mcmc_trace, experiments, prior_infor, args, nsamples=None, show_progress=True):
    """
...
def _map_running(trace, expts, prior_infor, shared_params, args):
    """
    Evaluate probability of a parameter set using posterior distribution
    Finding MAP (maximum a posterior) given prior distributions of parameters information
    ...
    [map_index, map_params, log_probs] = _map_finding(trace_map, expts, prior_infor, args, args.nsamples_MAP)
