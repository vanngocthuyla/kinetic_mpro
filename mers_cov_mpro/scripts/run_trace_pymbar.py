...
if args.multi_expt:
    ...
    for expt in expt_list:
        ...
        print("Running", expt)
        trace, flag = _trace_convergence(trace_files, 
                                         os.path.join(args.out_dir, expt), 
                                         nskip=args.nskip, nchain=args.nchain, nsample=args.niters, 
                                         key_to_check=args.key_to_check.split(), 
                                         converged_trace_name=args.converged_trace_name)
        if args.plotting: plotting_trace(trace, os.path.join(args.out_dir, expt), args.nchain)
        if not flag: unconverged_list.append('ASAP-00'+expt)
else:
    trace_files = [os.path.join(args.mcmc_dir, f, "traces.pickle") for f in mcmc_dir if os.path.isfile(os.path.join(args.mcmc_dir, f, "traces.pickle"))]
    trace, flag = _trace_convergence(trace_files, args.out_dir, 
                                     nskip=args.nskip, nchain=args.nchain, nsample=args.niters, 
                                     key_to_check=args.key_to_check.split(), 
                                     converged_trace_name=args.converged_trace_name)
    if args.plotting: plotting_trace(trace, args.out_dir, args.nchain)
    # if not flag: unconverged_list.append('ASAP-00'+expt)

if len(unconverged_list)>0: 
    print("Unconverged experiments:", unconverged_list)
    with open("Unconverged.txt", "w") as f:
        print("Unconverged experiments:", unconverged_list, file=f)
