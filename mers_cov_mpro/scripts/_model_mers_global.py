...
def global_fitting(experiments, prior_infor=None,
    ...
    if set_error_E and dE>0:
        error_E_list = {}
        error_E_list['dE:100'] = uniform_prior('dE:100', 0.567, 0.882)
        error_E_list['dE:50'] = uniform_prior('dE:50', 0.251, 0.792)
        error_E_list['dE:25'] = uniform_prior('dE:25', 0, 0.701)
        # error_E_list = _dE_priors(experiments, dE)
        # for conc in [100, 50, 25]:
        #     error_E_list[f'{conc}'] = uniform_prior(f'dE:{conc}', 0, dE)

    if multi_alpha:
        alphas = {}
        for i in range(4):
            alphas[i] = uniform_prior(f'alpha:ESI:{i}', lower=0, upper=2)
    ...
    for idx, expt in enumerate(experiments):
        ...
        if type(expt['kinetics']) is dict:
            for n in range(len(expt['kinetics'])):
                ...
                if data_rate is not None:
                    ...
                    fitting_each_dataset('kinetics', data_rate, [*_params_logK, *_params_kcat],
                                         alpha, percent_error_E, log_sigma, f'{idx_expt}:{n}')
        else:
            ...
            if data_rate is not None:
                ...
                fitting_each_dataset('kinetics', data_rate, [*_params_logK, *_params_kcat],
                                     alpha, percent_error_E, log_sigma, f'{idx_expt}')


def fitting_each_dataset(type_expt, data, params, alpha=None,
                         percent_error_E=None, log_sigma_rate=None, index=''):
    """
    Parameters:
    ----------
