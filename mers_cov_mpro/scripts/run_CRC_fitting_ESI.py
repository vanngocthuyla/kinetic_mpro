...
from _CRC_fitting import CRC_ESI_fitting
...
if len(expts_plot)>0:
    ...
    prior['logK_S_M'] = {'type':'logK', 'name': 'logK_S_M', 'fit':'global', 'dist': 'uniform', 'lower': -20.73, 'upper': logKd_max} 
    prior['logK_S_D'] = {'type':'logK', 'name': 'logK_S_D', 'fit':'global', 'dist': 'uniform', 'lower': -20.73, 'upper': logKd_max} 
    prior['logK_S_DS'] = {'type':'logK', 'name': 'logK_S_DS', 'fit':'global', 'dist': 'uniform', 'lower': -20.73, 'upper': logKd_max}
    ...
    if not os.path.isfile(traces_name+'.pickle'):
        if len(args.map_file)>0 and os.path.isfile(args.map_file):
            ...
            kernel = NUTS(model=CRC_ESI_fitting, init_strategy=init_to_value(values=init_values))
        else:
            kernel = NUTS(CRC_ESI_fitting)
...
from _CRC_fitting_ESI import global_fitting
from _load_data_mers import load_data_no_inhibitor, load_data_one_inhibitor
from _plotting import plot_data_conc_log, plotting_trace
from _MAP_finding_mers_concs import map_finding
...
prior = {}
prior['logKd'] = {'type':'logKd', 'name': 'logKd', 'fit':'global', 'dist': 'normal', 'loc': -10.133, 'scale': 0.481}
prior['logK_S_M'] = {'type':'logK', 'name': 'logK_S_M', 'fit':'global', 'dist': 'uniform', 'lower': -9.442, 'upper': -7} 
prior['logK_S_D'] = {'type':'logK', 'name': 'logK_S_D', 'fit':'global', 'dist': 'uniform', 'lower': -16.643, 'upper': -13.739} 
prior['logK_S_DS'] = {'type':'logK', 'name': 'logK_S_DS', 'fit':'global', 'dist': 'uniform', 'lower': -17.495, 'upper': -15.526}
prior['logK_I_M'] = {'type':'logK', 'name': 'logK_I_M', 'fit':'global', 'dist': 'uniform', 'lower': -20.73, 'upper': logKd_max} #1E-9
prior['logK_I_D'] = {'type':'logK', 'name': 'logK_I_D', 'fit':'global', 'dist': 'uniform', 'lower': logKd_min, 'upper': logKd_max}
prior['logK_I_DI'] = {'type':'logK', 'name': 'logK_I_DI', 'fit':'global', 'dist': 'uniform', 'lower': logKd_min, 'upper': logKd_max}
prior['logK_S_DI'] = {'type':'logK', 'name': 'logK_S_DI', 'fit':'global', 'dist': 'uniform', 'lower': logKd_min, 'upper': logKd_max}

prior['kcat_DS'] = {'type':'kcat', 'name': 'kcat_DS', 'fit':'global', 'dist': 'uniform', 'lower': 0.625, 'upper': 4.661}
prior['kcat_DSS'] = {'type':'kcat', 'name': 'kcat_DSS', 'fit':'global', 'dist': 'uniform', 'lower': 0.128, 'upper': 0.171}
prior['kcat_DSI'] = {'type':'kcat', 'name': 'kcat_DSI', 'fit':'global', 'dist': 'uniform', 'lower': kcat_min, 'upper': kcat_max}
...
if not os.path.isfile(traces_name+'.pickle'):
    ...
    if os.path.isfile(os.path.join(args.last_run_dir, "Last_state.pickle")):
        ...
        mcmc.run(mcmc.post_warmup_state.rng_key,  
                 experiments=expts, prior_infor=prior_infor_update, shared_params=shared_params,
                 multi_alpha=args.multi_alpha, multi_var=args.multi_var, 
                 set_K_S_DS_equal_K_S_D=args.set_K_S_DS_equal_K_S_D, 
                 set_K_S_DI_equal_K_S_DS=args.set_K_S_DI_equal_K_S_DS)
    else:
        mcmc = MCMC(kernel, num_warmup=args.nburn, num_samples=args.niters, num_chains=args.nchain, progress_bar=True)
...
E_list = {key: trace[key][map_index] for key in trace.keys() if key.startswith('dE')}
no_expt = len(expts_plot)
alphas = [trace[f'alpha:ESI:{i}'][map_index] for i in range(no_expt)]
