...
from _bayesian_model import global_fitting_jit
...
init_values = {'logKd': -0.005*jnp.ones(4), 'logK_S_M': -6.015*jnp.ones(4), 'logK_S_D': -9.312*jnp.ones(4), 
               'logK_S_DS': -13.853*jnp.ones(4), 'logK_I_M': jnp.zeros(4), 'logK_I_D': -17.669*jnp.ones(4), 
               'logK_I_DI': -18.141*jnp.ones(4), 'logK_S_DI': -13.967*jnp.ones(4),
               'kcat_MS': jnp.zeros(4), 'kcat_DS': jnp.zeros(4), 'kcat_DSI': 0.45*jnp.ones(4), 'kcat_DSS': 0.38*jnp.ones(4),
               }
...
kernel = NUTS(global_fitting_jit)
...
mcmc.run(rng_key=rng_key_, init_params=init_values,
         data_rate=[v, kinetics_logMtot, kinetics_logStot, kinetics_logItot], 
         data_AUC=[auc, AUC_logMtot, AUC_logStot, AUC_logItot], 
         data_ice=[ice, ice_logMtot, ice_logStot, ice_logItot],
         init_params_logK=params_logK, init_params_kcat=params_kcat)
mcmc.print_summary()

trace = mcmc.get_samples(group_by_chain=False)
pickle.dump(trace, open(os.path.join(args.out_dir, 'traces_init.pickle'), "wb"))

sample = az.convert_to_inference_data(mcmc.get_samples(group_by_chain=True))
az.plot_trace(sample)
plt.tight_layout();
plt.savefig(os.path.join(args.out_dir,'traces_init.pdf'))
plt.ioff()