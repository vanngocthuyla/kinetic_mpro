...
params_logK = {}
params_logK['logKd'] = -5.0 
for i in ['logK_S_M', 'logK_S_D', 'logK_S_DS', 'logK_I_M', 'logK_I_D', 'logK_I_DI', 'logK_S_DI']:
    params_logK[i] = None
params_kcat = {'kcat_MS': 0.0, 'kcat_DS': 0.0, 'kcat_DSI': None, 'kcat_DSS': None}
...
mcmc.run(rng_key_,
         ...
         params_logK, params_kcat)
mcmc.print_summary()

trace = mcmc.get_samples(group_by_chain=False)
pickle.dump(trace, open(os.path.join(args.out_dir, 'traces_fixed_kcat_Kd.pickle'), "wb"))

sample = az.convert_to_inference_data(mcmc.get_samples(group_by_chain=True))
az.plot_trace(sample)
plt.tight_layout();
plt.savefig(os.path.join(args.out_dir,'traces_fixed_kcat_Kd.pdf'))
plt.ioff()