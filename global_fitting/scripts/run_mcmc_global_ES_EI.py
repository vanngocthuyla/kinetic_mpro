...
os.environ['TF_XLA_FLAGS'] = '--xla_dump_to=/tmp/foo'

warnings.simplefilter(action='ignore', category=FutureWarning)
warnings.simplefilter("ignore", UserWarning)
warnings.simplefilter("ignore", RuntimeWarning)

from _bayesian_model import global_fitting
...
mcmc.run(rng_key_, experiments_combined, params_logK, params_kcat)
mcmc.print_summary()

trace = mcmc.get_samples(group_by_chain=False)
pickle.dump(trace, open(os.path.join(args.out_dir, 'traces_ES_EI.pickle'), "wb"))

sample = az.convert_to_inference_data(mcmc.get_samples(group_by_chain=True))
az.plot_trace(sample)
plt.tight_layout();
plt.savefig(os.path.join(args.out_dir,'traces_ES_EI.pdf'))
plt.ioff()

with open("summary.txt", "w") as f:
    print(str(mcmc.print_summary()), file=f)