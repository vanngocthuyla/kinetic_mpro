...
def analytical_binding_enzyme_substrate(logMtot, logStot, logKd, logK_S_M, logK_S_D, logK_S_DS):
    """
    Analytical solution to compute equilibrium concentrations of species 
    for a binding model of an enzyme and a substrate
    ...
    Stot = np.exp(logStot)
    Ka = 1/jnp.exp(logKd)
    K_S_M = 1/jnp.exp(logK_S_M)
    Ka_MS_M = 1/jnp.exp(logKd_MS_M)
    K_S_DS = 1/jnp.exp(logK_S_DS)

    y = jnp.zeros(logMtot.shape[0], jnp.float64)
    for i in range(len(Mtot)):
        M0 = Mtot[i]
        S0 = Stot[i]
        b0 = 4*Ka**2*S0**2
        b1 = -8*Ka**2*S0 + 2*Ka*K_S_M*S0 - Ka_MS_M*K_S_M*S0 - 4*Ka*Ka_MS_M*K_S_M*M0*S0 + 8*Ka*Ka_MS_M*K_S_M*S0**2
        b2 = 4*Ka**2 - 2*Ka*K_S_M + Ka_MS_M*K_S_M + 4*Ka*Ka_MS_M*K_S_M*M0 - 2*Ka*K_S_M**2*M0 + Ka_MS_M*K_S_M**2*M0 + \
            Ka_MS_M**2*K_S_M**2*M0**2 - 16*Ka*Ka_MS_M*K_S_M*S0 + 2*Ka*K_S_M**2*S0*- 2*Ka_MS_M*K_S_M*K_S_DS*S0 - \
            4*Ka_MS_M**2*K_S_M**2*M0*S0 - 8*Ka*Ka_MS_M*K_S_M*K_S_DS*M0*S0 + 4*Ka_MS_M**2*K_S_M**2*S0**2 + \
            8*Ka*Ka_MS_M*K_S_M*K_S_DS*S0**2
        
        if S0 > M0:
            y = y.at[i].add((S0-M0)*(1+M0/S0))
        else:
            Delta = b1**2 - 4*b2*b0
            y = y.at[i].add((-b1 - jnp.sqrt(Delta))/(2*b2))
    
    x = (-1 + jnp.sqrt(1-4*(-Mtot+Stot-y)*(2*Ka+Ka_MS_M*K_S_M*y) ))/(2*(2*Ka+Ka_MS_M*K_S_M*y))

    species = ['M','D','I','S','MS','MI','DI','DII','DS','DSI','DSS']
    log_concs = dict([(key, jnp.log(jnp.ones(logMtot.shape[0], jnp.float64)*1E-20)) for key in species])
    log_concs['M'] = jnp.log(x)
    log_concs['D'] = 2*log_concs['M'] - logKd
    log_concs['S'] = jnp.log(y)
    log_concs['MS'] = log_concs['M'] + log_concs['S'] - logK_S_M
    log_concs['DS'] = 2*log_concs['M'] + log_concs['S'] - logKd_MS_M - logK_S_M
    log_concs['DSS'] = 2*log_concs['M'] + 2*log_concs['S'] - logKd_MS_M - logK_S_M - logK_S_DS

    return log_concs


def analytical_binding_enzyme_inhibitor(logMtot, logItot, logKd, logK_I_M, logK_I_D, logK_I_DI):
    """
    Analytical solution to compute equilibrium concentrations of species 
    for a binding model of an enzyme and a inhibitor
    ...
    Itot = np.exp(logItot)
    Ka = 1/jnp.exp(logKd)
    K_I_M = 1/jnp.exp(logK_I_M)
    Ka_MI_M = 1/jnp.exp(logKd_MI_M)
    K_I_DI = 1/jnp.exp(logK_I_DI)

    z = jnp.zeros(logMtot.shape[0], jnp.float64)
    for i in range(len(Mtot)):
        M0 = Mtot[i]
        I0 = Itot[i]
        b0 = 4*Ka**2*I0**2
        b1 = -8*Ka**2*I0 + 2*Ka*K_I_M*I0 - Ka_MI_M*K_I_M*I0 - 4*Ka*Ka_MI_M*K_I_M*M0*I0 + 8*Ka*Ka_MI_M*K_I_M*I0**2
        b2 = 4*Ka**2 - 2*Ka*K_I_M + Ka_MI_M*K_I_M + 4*Ka*Ka_MI_M*K_I_M*M0 - 2*Ka*K_I_M**2*M0 + Ka_MI_M*K_I_M**2*M0 + \
            Ka_MI_M**2*K_I_M**2*M0**2 - 16*Ka*Ka_MI_M*K_I_M*I0 + 2*Ka*K_I_M**2*I0*- 2*Ka_MI_M*K_I_M*K_I_DI*I0 - \
            4*Ka_MI_M**2*K_I_M**2*M0*I0 - 8*Ka*Ka_MI_M*K_I_M*K_I_DI*M0*I0 + 4*Ka_MI_M**2*K_I_M**2*I0**2 + \
            8*Ka*Ka_MI_M*K_I_M*K_I_DI*I0**2

        if I0 > M0:
            z = z.at[i].add((I0-M0)*(1+M0/I0))
        else:
            Delta = b1**2 - 4*b2*b0
            z = z.at[i].add((-b1 - jnp.sqrt(Delta))/(2*b2))
    
    x = (-1 + jnp.sqrt(1-4*(-Mtot+Itot-z)*(2*Ka+Ka_MI_M*K_I_M*z) ))/(2*(2*Ka+Ka_MI_M*K_I_M*z))

    species = ['M','D','I','S','MS','MI','DI','DII','DS','DSI','DSS']
    log_concs = dict([(key, jnp.log(jnp.ones(logMtot.shape[0], jnp.float64)*1E-20)) for key in species])
    log_concs['M'] = jnp.log(x)
    log_concs['D'] = 2*log_concs['M'] - logKd
    log_concs['I'] = jnp.log(z)
    log_concs['MI'] = log_concs['M'] + log_concs['I'] -  logK_I_M
    log_concs['DI'] = 2*log_concs['M'] + log_concs['I'] - logKd_MI_M - logK_I_M
    log_concs['DII'] = 2*log_concs['M'] + 2*log_concs['I'] - logKd_MI_M - logK_I_M -logK_I_DI

    return log_concs


def analytical_ReactionRate(logMtot, logStot, logItot, 
                            logKd, logK_S_M, logK_S_D, logK_S_DS, logK_I_M, logK_I_D, logK_I_DI, logK_S_DI,
                            kcat_MS=0., kcat_DS=0., kcat_DSI=1., kcat_DSS=1.):
    """
    Reaction Rate
    ...
    """
    if logItot is None: 
        log_concs = analytical_binding_enzyme_substrate(logMtot, logStot, 
                                                        logKd, logK_S_M, logK_S_D, logK_S_DS)
    elif logStot is None:
        log_concs = analytical_binding_enzyme_inhibitor(logMtot, logItot, 
                                                        logKd, logK_I_M, logK_I_D, logK_I_DI)
    else: 
        log_concs = DimerBindingModel(logMtot, logStot, logItot, 
                                      logKd, logK_S_M, logK_S_D, logK_S_DS, 
                                      logK_I_M, logK_I_D, logK_I_DI, logK_S_DI)
...
def analytical_MonomerConcentration(logMtot, logStot, logItot, 
                                    logKd, logK_S_M, logK_S_D, logK_S_DS, logK_I_M, logK_I_D, logK_I_DI, logK_S_DI):
    """
    Response of MonomerConcentration ~ [M] + [MI] + [MI]
...
def analytical_CatalyticEfficiency(logMtot, logItot, 
                                   logKd, logK_S_M, logK_S_D, logK_S_DS, logK_I_M, logK_I_D, logK_I_DI, logK_S_DI,
                                   kcat_MS=0., kcat_DS=0., kcat_DSI=1., kcat_DSS=1., 
                                   logStot = None):
    """
    ...
    catalytic_efficiency = jnp.zeros(logItot.shape, jnp.float32)
    v1 = analytical_ReactionRate(logMtot, jnp.ones(logItot.shape[0])*logStot[0], logItot,
                                 logKd, logK_S_M, logK_S_D, logK_S_DS, logK_I_M, logK_I_D, logK_I_DI, logK_S_DI, 
                                 kcat_MS, kcat_DS, kcat_DSI, kcat_DSS)
    v2 = analytical_ReactionRate(logMtot, jnp.ones(logItot.shape[0])*logStot[1], logItot,
                                 logKd, logK_S_M, logK_S_D, logK_S_DS, logK_I_M, logK_I_D, logK_I_DI, logK_S_DI, 
                                 kcat_MS, kcat_DS, kcat_DSI, kcat_DSS)
    catalytic_efficiency = (v2-v1)/DeltaS
    return catalytic_efficiency